...
int main(int argc, char *argv[]) { //main function
    FILE *input; //Create an input file
    if (argc == 2) { //Take one input file per each execution of the program
        input = fopen(argv[1], "r"); //Read mode activated for the input file
    } else {
        printf("Please enter input files one by one.");
        exit(0);
    }
